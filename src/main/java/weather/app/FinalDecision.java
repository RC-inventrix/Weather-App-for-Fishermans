...
public class FinalDecision {
    public static void finalDecision(Inputs inputdata) {
        ...
        if (Comparing.compare(data) == -1) {
            ...
            if (RealMaxDays.realMaxDays(inputdata) == 0) {
                System.out.println(
                        "Unfortunately based on the weather condition of the rest of the day, you cannot stay for the whole day.");
            } else if (RealMaxDays.realMaxDays(inputdata) < inputdata.getMaxDays()) {
                System.out
                        .println("You can only stay safely for only " + RealMaxDays.realMaxDays(inputdata) + " days.");
                Calculator.Consumption(inputdata, RealMaxDays.realMaxDays(inputdata));
            } else {
